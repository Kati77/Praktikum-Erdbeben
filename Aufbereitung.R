...
cali_daten <- mutate(cali_daten, triggeringMag = 0)
for(i in 1:length(cali_daten$triggeredFrom)){
  if(cali_daten$triggeredFrom[i] == -1)  cali_daten$triggeringMag[i] <- -1
  else {
    evID <- cali_daten$triggeredFrom[i]
    cali_daten$triggeringMag[i] <- cali_daten$mag[evID]
  }
}
remove(evID)
...
cali_daten <- cali_daten %>%
  rename(triggeredMag = mag)

cali_reg <- filter(cali_daten, triggeringMag != -1)
